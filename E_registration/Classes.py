class Course:
    ...
    LevelCourses = {"Beginner": [], "Intermediate": [], "Advanced": []}

    def __init__(
        self, CId: int, CName: str, CPrice: int, CLevel: str, Chours: int
    ) -> None:
...
class Student:
    ...
    def __init__(
        self, Std_Id: int, Std_Name: str, Std_Level: str, Std_Wallet: int
    ) -> None:
        self.id = Std_Id
    ...
    def getSchedule(self):
        ...
        for i in range(5):
            for j in range(7):
                if (
                    schedule[i][j]
                    and schedule[i][j] not in __class__.added_courses_names
                ):
                    schedule[i][j] = ""
        return schedule
