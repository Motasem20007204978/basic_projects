...
while True:
    ...
    Choices = ['1.Register new student', '2.Register new course', 
                '3.Register course for student', '4.Delete course for student', 
                'Print student schedule', '6.Exit system']
    print("\n".join(Choices))
    response = int(click.prompt('choose ', type=click.Choice([str(x) for x in range(1, len(Choices) + 1)])))
    ...
    if response == 1:
        ...
        print("\nchoose level:\n" + "\n".join(Levels.values()))
        levnum = int(click.prompt('choose ', type=click.Choice([str(x) for x in range(1, len(Levels.keys()) + 1)])))
        level = Levels[levnum]
        ...
        for hour in range(1, hours+1):            
            while True:
                ...
                print('\nDays:\n' + "\n".join(Days))
                book_day = int(click.prompt('choose ', type=click.Choice([str(x) for x in range(1, len(Days) + 1)])))
                
                print('\nChoose time for lecture ', hour, 'of this course')
                print('\nTimes:\n' + "\n".join(Hours))
                book_hour = int(click.prompt('choose ', type=click.Choice([str(x) for x in range(1, len(Hours) + 1)])))
        ...
        print('\n', chosen_course.getName(), ' course is added successfully for student ', chosen_student.getName(), '\n')
        ...
        ID = int(click.prompt('\nChoose course ID', type=click.Choice([str(i) for i in Student.added_courses_ids])))
        ...
        print('\ncourse ', chosen_course.getName(), ' is deleted for student ', chosen_student.getName(), '\n')
        ...
        table = tabulate(schedule, headers='firstrow', tablefmt='fancy_grid', showindex=Days)
        print(table)
