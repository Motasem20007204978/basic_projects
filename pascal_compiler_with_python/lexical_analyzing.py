...
class LexeicalAnalyzer:
        ...
        while line:
            ...
            if line.startswith('"') or line.startswith("'"):
                try:
                    ...
                    print(
                        "Error: Lexical Error at line {}".format(lineNo)
                        + "\n"
                        + line
                        + "\n"
                        + " " * (len(line) + 1)
            ...
            if not white_space:
                line = exportLexeme(line=line, pattern=r"^[\W]")  # non-alpha lexemes
    ...
    def __str__(self):
        ...
        for record in self.__symbol_list:
            _str += (
                record._getName().ljust(20)
                + "| "
                + record._getValue().ljust(50)
                + "| "
                + str(record._getLineNo())
                + "\n"
            )
            _str += "-" * 100 + "\n"
